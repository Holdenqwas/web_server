...
async def create_user(data: users_schema.CreateUser,
...
async def update_date_license(data: users_schema.UpdateDateLicense,
                              # user: str = Depends(require_user),
                              db: AsyncSession = Depends(get_db)):
...
async def create_trainings(data: users_schema.NameTrainings,
                           # user: str = Depends(require_user),
                           db: AsyncSession = Depends(get_db)):
...
async def update_name_exercises(data: users_schema.NameExercise,
                                # user: str = Depends(require_user),
                                db: AsyncSession = Depends(get_db)):
...
@router.delete('/delete_trainings/{user_name}', response_model=users_schema.User)
async def delete_trainings(user_name: str,
                           # user: str = Depends(require_user),
                           db: AsyncSession = Depends(get_db)):
...
async def create_exercises(data: users_schema.NameTrainings,
                           # user: str = Depends(require_user),
                           db: AsyncSession = Depends(get_db)):
    result = await crud.update_name_exercises(data, db)
