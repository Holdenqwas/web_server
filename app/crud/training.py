...
async def get_name_trains(user_id: str, db: AsyncSession) -> List[str]:
...
async def get_name_exercise(data: schema.NameExercises, db: AsyncSession):
    ...
    name_trains = await get_name_trains(data.user_id, db)
...
async def get_training(user_id: str, db: AsyncSession):
    stmt = (
        select(db_model.TrainingAll)
        .where(db_model.TrainingAll.user_id == user_id)
...
async def get_prev_training(user_id: str, index_train, db: AsyncSession):
    stmt = (
        select(db_model.TrainingAll)
        .where(
            and_(
...
async def create_training_all(data: schema.CreateTrainingAll, db: AsyncSession):
    ...
    db.add(new_train)
    new_train.name_trains = await get_name_trains(data.user_id, db)
    return new_train
...
async def create_train(data: schema.CreateTrain, db: AsyncSession):
    ...
    train_all = await get_training(data.user_id, db)
